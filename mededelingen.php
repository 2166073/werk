...
// Haal gebruikers op voor het dropdown-menu (inclusief email)
$gebruikers = $pdo->query("SELECT gebruiker_id, email, rol FROM gebruiker ORDER BY rol")->fetchAll(PDO::FETCH_ASSOC);
?>
                    ...
                    <select name="gebruiker_id" id="gebruiker_id" class="form-select" required>
                        ...
                        <?php foreach ($gebruikers as $gebruiker): ?>
                            <option value="<?= $gebruiker['gebruiker_id'] ?>">
                                <?= htmlspecialchars($gebruiker['email']) ?> (<?= htmlspecialchars($gebruiker['rol']) ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
